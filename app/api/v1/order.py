...
async def get_orders(user_id: int, db: AsyncSession = Depends(get_async_session)):
...
async def get_order(user_id: int, order_id: str, db: AsyncSession = Depends(get_async_session)):
    result = await db.execute(
        select(Order).where(
