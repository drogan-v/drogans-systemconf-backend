...
@router.get(
    "/orders/{user_id}",
    response_model=List[OrderResponse],
    description="Get all orders of user by user_id",
...
@router.get(
    ...
    description="Get order of user by user_id and order_id",
    summary="Get order of user",
)
async def get_order(
    user_id: int, order_id: str, db: AsyncSession = Depends(get_db_session)
):
